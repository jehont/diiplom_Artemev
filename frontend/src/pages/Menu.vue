      ...
      <div class="col-sm-4 col-12 filter-box">
        ...
        <div class="row filter-drop-down">
          <p @click="displayFilterDrop">
            Filter<span v-if="showDropDown">x</span><span v-else>v</span>
          </p>
        </div>
      ...
      <div class="col-sm-8">
        <div class="row">
          <div class="menu-tabs">
            ...
            <input
              type="button"
              id="soupFilterFoodBtn"
              name="soupFilterFoodBtn"
              class="menu-tab-item"
              value="Супы"
              @click="filterFoodBtn($event)"
            />
            <input
              type="button"
              id="salatFilterFoodBtn"
              name="salatFilterFoodBtn"
              class="menu-tab-item"
              value="Салаты"
              @click="filterFoodBtn($event)"
            />
            <input
              type="button"
              id="zakuskaFilterFoodBtn"
              name="zakuskaFilterFoodBtn"
              class="menu-tab-item"
              value="Мясо & рыба"
              @click="filterFoodBtn($event)"
            />
            <input
              type="button"
              id="garnirFilterFoodBtn"
              name="garnirFilterFoodBtn"
              class="menu-tab-item"
              value="Гарниры"
              @click="filterFoodBtn($event)"
            />
        ...
        <div class="row box-container">
          <div v-for="(f, index) in currentPageItems" :key="index">
            <div class="box">
              ...
              <div class="content">
                ...
                <button class="btn" @click="addItem(index)">Добавить в корзину</button>
              </div>
            </div>
          </div>
          <div v-if="!filterFoods.length">
            <div class="box">
              <div class="content">
                <h1 style="color: #057835fa">Совпадение не найдено!</h1>
              </div>
              <div class="image">
...
export default {
      ...
      return this.allFoods.filter(
        (f) =>
          ...
          (f.food_category.match(this.foodObj.category) ||
            this.foodObj.category == "Полное меню" ||
  ...
  methods: {
    ...
    checkSale: function (food, statusArray) {
      if (statusArray.includes("Скидка")) {
    ...
    checkBest: function (food, statusArray) {
      if (statusArray.includes("популярное")) {
        if (food.food_status.includes("популярное")) {
          return true;
        } else {
          return false;
        }
    ...
    checkOnl: function (food, statusArray) {
      if (statusArray.includes("только онлайн")) {
        if (food.food_status.includes("только онлайн")) {
          return true;
        } else {
          return false;
        }
    ...
    checkSeason: function (food, statusArray) {
      if (statusArray.includes("сезонное блюдо")) {
        if (food.food_status.includes("сезонное блюдо")) {
          return true;
        } else {
          return false;
        }
    ...
    checkNew: function (food, statusArray) {
      if (statusArray.includes("Новое блюдо")) {
        if (food.food_status.includes("Новое блюдо")) {
          return true;
        } else {
          return false;
        }
    ...
    evaluatePrice: function (food, priceRange) {
      ...
      } else if (priceRange == "410,800") {
        if (410 <= cal && cal <= 800) {
          return food;
        }
      } else if (priceRange == "810,1500") {
        if (810 <= cal && cal <= 1500) {
          return food;
        }
